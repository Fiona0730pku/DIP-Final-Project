...
def train():
    ...
    with tf.Session(graph=graph) as sess:
        ...
        try:           
            for train_epoch in range(0,6000):
                batch_xs = []
                batch_ys = []
                for i in range(0,1016):
                    ...
                    if i == 0:
                        ...
                        batch_ys = np.array([int(j == num-1) for j in range(62)])[np.newaxis,:]
                    else:
                        batch_xs = np.concatenate((batch_xs,parse(files[tmp+i])),axis = 0)
                        #print(batch_ys.shape)
                        #print(tf.one_hot(num-1, 62)[np.newaxis,:].shape)
                        batch_ys = np.concatenate((batch_ys,(np.array([int(j == num-1) for j in range(62)])[np.newaxis,:])),axis = 0)
                #print(batch_xs[0])
                #print(batch_ys[0])
                tmp += 1016
                if tmp == 1016*62:
